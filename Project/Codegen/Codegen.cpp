...
{
	CPathMgr::init();
	wstring wcsProjectPath = CPathMgr::GetProjectPath();
	wstring wcsScriptPath = wcsProjectPath + L"Scripts";
	wstring wcsHeaderPath = wcsProjectPath + L"Scripts\\CScriptMgr.h";
	wstring wcsCppPath = wcsProjectPath + L"Scripts\\CScriptMgr.cpp";


	WIN32_FIND_DATA tData = {};
	HANDLE handle = FindFirstFile(wstring(wcsScriptPath + L"\\*.h").c_str(),&tData);
	...
	while (true)
	{
		if (wcscmp(tData.cFileName, L"stdafx.h")
			&& wcscmp(tData.cFileName, L"CScriptMgr.h")
			&& wcscmp(tData.cFileName, L"targetver.h")
			&& wcscmp(tData.cFileName, L"CUnitScript.h")
			&& wcscmp(tData.cFileName, L"CMgrScript.h")
			&& wcscmp(tData.cFileName, L"CSaveLoadMgr.h")
			)
		{
	...
	// ScriptMgr h 작성
	//=================
	_wfopen_s(&pFile, wcsHeaderPath.c_str(), L"w");
	...
	// ScriptMgr cpp 작성
	//====================
	_wfopen_s(&pFile, wcsCppPath.c_str(), L"w");
